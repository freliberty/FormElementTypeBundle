...
!function ($) {
    ...
    var SpinEdit = function (element, options) {
        ...
        this.btnFaClass = $.fn.spinedit.defaults;
        if (hasOptions && typeof options.btnFaClass == 'string') {
            this.setBtnFaClass(options.btnFaClass);
        }
        ...
        var templatebefore = $(this.getTemplatebefore());
        var templateafter = $(this.getTemplateafter());

        this.element.before(templatebefore);
        this.element.after(templateafter);
        this.element.addClass('spinedit-input');

        $(templatebefore).each(function (i,x) {
            $(x).bind('selectstart click mousedown', function () { return false; });
        });
        $(templateafter).each(function (i,x) {
            $(x).bind('selectstart click mousedown', function () { return false; });
        });

        templateafter.find('.fa-plus-'+this.btnFaClass).mousehold($.proxy(this.increase, this));
        templatebefore.find('.fa-minus-'+this.btnFaClass).mousehold($.proxy(this.decrease, this));
    ...
    SpinEdit.prototype = {
        ...
        setBtnFaClass: function (value) {
            this.btnFaClass = value;
        },

        getBtnFaClass: function () {
            return this.btnFaClass;
        },

        getTemplatebefore: function() {
            return '<div class="spinedit spinedit-before">' +
            '<i class="fa fa-minus-'+this.getBtnFaClass()+' fa-lg fa-2x spinedit-action main-color"></i>' +
            '</div>';
        },

        getTemplateafter: function() {
            return '<div class="spinedit spinedit-after">' +
            '<i class="fa fa-plus-'+this.getBtnFaClass()+' fa-lg fa-2x spinedit-action main-color"></i>' +
            '</div>';
        },
        ...
        setNumberOfDecimals: function (value) {
            this.numberOfDecimals = parseInt(value, 10);
        },
